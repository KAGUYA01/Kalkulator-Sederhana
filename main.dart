...
class HomePage extends StatelessWidget {
  ...
  void calculateInputs() {
    if (tempInputs.isNotEmpty) {
      ...
      final mainNumbers = tempNumbers
          .map((e) => e.contains('0') ? double.parse(e) : int.parse(e))
          .toList();
  ...
  Widget build(BuildContext context) {
    ...
    final Map<String, Color> charColors = {
      'AC': Colors.redAccent.shade200,
      'Del': Colors.redAccent.shade200,
      '÷': Colors.greenAccent,
      '×': Colors.greenAccent,
      '+': Colors.greenAccent,
      '-': Colors.greenAccent,
                                ...
                                child: InkWell(
                                  onTap: () {
                                    if (char.isNumber) {
                                      ...
                                      int startSub = tempInputs.lastIndexWhere(
                                          (e) =>
                                              ['÷', '×', '-', '+'].contains(e));
                                      ...
                                      if (startSub != -1) {
                                        subTempInput = tempInputs.sublist(
                                            startSub + 1, endSub + 1);
                                      }

                                      if (!subTempInput.contains('.')) {
                                        if (subTempInput.isNotEmpty &&
                                            subTempInput.last.isNumber) {
                                          tempInputs.add(char);
                                        }
                                      }
                                    } else if (['÷', '×', '-', '+']
